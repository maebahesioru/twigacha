    ...
    return (
      <div className={`min-h-dvh bg-gray-950 text-white py-10 px-4 flex flex-col items-center gap-6 slide-in-up ${raidResult.cleared ? 'raid-clear-flash' : ''}`}>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 w-full max-w-2xl">
          ...
          <button onClick={() => { window.open(`https://misskeyshare.link/share.html?text=${encodeURIComponent(copyText)}&url=${encodeURIComponent('https://twigacha.vercel.app')}`, '_blank'); import('@/store/useGameStore').then(m => m.useGameStore.getState().markShare()); }} className="px-5 py-3 bg-cyan-600 rounded-xl font-bold hover:bg-cyan-500 transition">Misskey</button>
          <button onClick={() => { window.open(`https://donshare.net/share.html?text=${encodeURIComponent(copyText)}&url=${encodeURIComponent('https://twigacha.vercel.app')}`, '_blank'); import('@/store/useGameStore').then(m => m.useGameStore.getState().markShare()); }} className="px-5 py-3 bg-indigo-600 rounded-xl font-bold hover:bg-indigo-500 transition">Mastodon</button>
