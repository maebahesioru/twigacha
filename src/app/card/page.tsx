...
export default function CardSearchPage() {
  ...
  return (
    <div className="min-h-dvh bg-gray-950 text-white flex flex-col items-center justify-center px-4 gap-6">
      <h1 className="text-2xl font-black text-pink-400">🔍 カードを検索</h1>
      <p className="text-gray-400 text-sm">Twitter ID または Bluesky ハンドルを入力</p>
      <div className="flex gap-2 w-full max-w-sm">
        <input
          value={input}
          onChange={e => setInput(e.target.value)}
          onKeyDown={e => e.key === "Enter" && go()}
          placeholder="username または user.bsky.social"
