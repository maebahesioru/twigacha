...
export default function TcgCard({ card, size = "md", onClick, selected }: Props) {
  ...
  return (
    <div
      onClick={() => { if (onClick === undefined) return; const isMastodon = card.id?.startsWith('mdon_'); const isMisskey = !isMastodon && (card.id?.startsWith('msky_') || card.username.includes('@')); const isBsky = !isMisskey && !isMastodon && card.username.includes('.'); const url = isMastodon ? `https://${card.username.includes('@') ? card.username.split('@')[1] : 'mastodon.social'}/@${card.username.split('@')[0]}` : isMisskey ? `https://${card.username.includes('@') ? card.username.split('@')[1] : 'misskey.io'}/@${card.username.split('@')[0]}` : isBsky ? `https://bsky.app/profile/${card.username}` : `https://twitter.com/${card.username}`; window.open(url, '_blank', 'noopener,noreferrer'); onClick(); }}
      className={`
